...
export async function GET(req: NextRequest) {
  try {
    ...
    const status = AGENCY_STATUSES.includes(
      statusParam as (typeof AGENCY_STATUSES)[number],
    )
      ? (statusParam as (typeof AGENCY_STATUSES)[number])
      : "";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const agency = await createAgency(validation.data, actor.email);
    ...
    return NextResponse.json({ error: "agency_create_failed" }, { status: 500 });
  }
}
